...
class CityService {
  public static async getCityWeather(
    cityName: string
  ): Promise<CityListItemProps> {
    const cityData = (
      await weaterApi.get(`/weather?q=${cityName}&appid=${OPENWEATHER_API_KEY}`)
    ).data;
  ...
  public static async getCitiesWeather(
    ...
    for (let i = 0; i < citiesNames.length; i++) {
      const cityData = (
        await weaterApi.get(
          `/weather?q=${citiesNames[i]}&appid=${OPENWEATHER_API_KEY}`
        )
      ).data;
